...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
	...
	<table class="table table-borderless table-hover">
		<thead>
			<tr class="border-bottom font-weight-bold">
			 	...
			 	<th class="text-right">등급수정</th>
			 	<th class="text-right">비밀번호수정</th>
			 	<th class="text-right">강제탈퇴</th>
			</tr>
		</thead>
		<tbody>
			<%
			...
			%>
			<tr class="border-bottom font-weight-bold">
				<td class="text-right"><%=m.getMemberNo()%></td>
				<td class="text-right"><%=m.getMemberId()%></td>
				<td class="text-right">
				...
				<td class="text-right"><%=m.getMemberName()%></td>
				<td class="text-right"><%=m.getMemberAge()%></td>
				<td class="text-right"><%=m.getMemberGender()%></td>
				<td class="text-right"><%=m.getUpdateDate()%></td>
				<td class="text-right"><%=m.getCreateDate()%></td>
				<td class="text-right">
					<!-- 특정회원의 등급을 수정 -->
	...
	<%
		...
		if (currentPage > 1) {
	%>
		<a class="btn btn-outline-info" href="<%=request.getContextPath()%>/admin/selectMemberList.jsp?currentPage=<%=currentPage-1%>&searchMemberId=<%=searchMemberId%>">이전</a>
	<%
		}
	%>
	<%		
	...
	%>
		<a class="btn btn-outline-info" href="<%=request.getContextPath()%>/admin/selectMemberList.jsp?currentPage=<%=currentPage+1%>&searchMemberId=<%=searchMemberId%>">다음</a>
	<%
	}
	%>
	</div>
	...
	<div>
		<form action="<%=request.getContextPath()%>/admin/selectMemberList.jsp" method="get">
			memberId :
			<input class="text-center" type="text" name="searchMemberId">
